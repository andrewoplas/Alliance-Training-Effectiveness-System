...
!function($) {
    ...
    CalendarApp.prototype.init = function() {
        ...
        var dateStarts = $('.dateStarts').val().split(',');
        	dateStarts.splice(0, 1);
        	
        var dateEnds = $('.dateEnds').val().split(',');
        	dateEnds.splice(0, 1);
        	
    	var classNames = $('.classNames').val().split(',');
    		classNames.splice(0, 1);
       
        var dateTemp;
        var defaultEvents =  [];
        var length = dateStarts.length;
        for(var i=0; i<length; i++) {
        	dateTemp = dateStarts[i].split(':');  
        	dateStarts[i] = new Date(dateTemp[0], dateTemp[1]-1, dateTemp[2], dateTemp[3], dateTemp[4]);
        	
        	dateTemp = dateEnds[i].split(':');  
        	dateEnds[i] = new Date(dateTemp[0], dateTemp[1]-1, dateTemp[2], dateTemp[3], dateTemp[4]);
